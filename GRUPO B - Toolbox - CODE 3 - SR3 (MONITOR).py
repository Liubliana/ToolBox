#Lucas Marinho (lmr) é o único cadastrado como monitor
#Senha: 3344
...
def report(arquivo_erro, report, erro):
    texto = report + data_e_hora.strftime('%d/%m/%Y %H:%M') + ' - ' + string_linha[
        0] + ', ' + matricula + ':  ' + erro + '\n\n'
    ...
    while acesso != 0:
        ...
        if acesso == 1:
            for linha in senhas:
                ...
                if iniciais == string_linha[4] and '#Monitor' in linha:
                        ...
                        if classes[op] == classes[i]:
                            ...
                            if ferramenta_opcao in classes[op]:
                                ...
                                while B == 0:
                                    ...
                                    if menu_opcao == 1:
                                        print('Você selecionou retirar a ferramenta',
                                              classes[op][ferramenta_opcao]['nome'], ', deseja continuar? (s/n)')
                                        sn = input()
                                        if sn == 'n' or sn == 'N':
                                            continue
                                        elif sn == 's' or sn == 'S':
                                            ...
                                            print('Há ' + str(
                                                classes[op][ferramenta_opcao]['quantidade']) + ' para a retirada')
                                            pergunta = 'Quantas ferramentas "' + classes[op][ferramenta_opcao][
                                                'nome'] + '" deseja retirar? '
                                            resposta = 'Deve ser um número inteiro!'
                                            ...
                                            if qtdd_sobra >= 0:
                                                retirar2()
                                                classes[op][ferramenta_opcao][
                                                    'quantidade'] = qtdd_disponivel - qtdd_retirada
                                        ...
                                        if classes[op][ferramenta_opcao]['nome'] in linhas_carrinho_str:
                                            if qtdd_devolvida <= quantidade_l:
                                                ...
                                                devolver(nome_arquivo_al, classes[op][ferramenta_opcao]['nome'],
                                                         classes[op][ferramenta_opcao]['nome'], e, qtdd_devolvida)

                                                devolver('Histórico_geral.txt', nome_aluno,
                                                         classes[op][ferramenta_opcao]['nome'], f, qtdd_devolvida)

                                                devolver(nome_arquivo_f, nome_aluno, nome_aluno, g, qtdd_devolvida)

                                                classes[op][ferramenta_opcao][
                                                    'quantidade'] = qtdd_disponivel + qtdd_devolvida
                                                classes = dicionario(classes)
                                        ...
                                        print(classes[op][ferramenta_opcao]['nome'] + ': ' +
                                              classes[op][ferramenta_opcao]['como usar'])
                    ...
                    if b == 0:
                        ...
                        if agend == 1:
                            ...
                            while g == 0:
                                ...
                                print('Você selecionou a data {}, hora: {}, confirma? (s/n)'.format(dia_escolhido,
                                                                                                    hora_escolhida))
                                sn = input()
                                if sn == 's':
                                    arquivo = open('Agendamentos_Impressora3D.txt', 'a', encoding='utf8')
                                    texto = dia_escolhido + ' ' + hora_escolhida + ' - ' + string_linha[1] + ' ' + \
                                            string_linha[2] + '  ' + string_linha[8] + '\n\n'
                                    arquivo.write(texto)
                                    arquivo.close()
